...
import { addDays, allDayEventsOnDay, buildEventsByDay, formatCalendarPeriodTitle, monthCells, startOfDay, startOfWeekMonday, startOfWeekSunday, timedEventsOnDay, timedSegmentPercentOnDay, toLocalDateKey, } from './calendar-utils.js';
import { CALENDAR_MOBILE_MAX_PX, CAL_MIN_HEIGHT, calendarViewOptionsForMobile, DAY_TIMELINE_HEIGHT_PX, HOUR_ROW_PX, rangeStyleForEventId, WEEKDAYS_SUN, WORK_HEADERS, } from './notion-calendar-constants.js';
...
.nec-toolbar-mobile-head{display:flex;flex-direction:column;align-items:flex-start;gap:6px;width:100%}
...
.nec-toolbar-row{display:flex;flex-wrap:wrap;align-items:center;gap:8px;width:100%}
.nec-toolbar-desktop-top{display:flex;flex-wrap:wrap;align-items:flex-start;justify-content:space-between;gap:12px;width:100%}
.nec-toolbar-controls{display:flex;flex-wrap:wrap;align-items:center;gap:8px}
.nec-view-tabs{display:inline-flex;border-radius:2px;border:1px solid var(--nec-control-border);background:var(--nec-week-header-bg);padding:2px;gap:0}
...
.nec-scroller-mobile{overflow-x:auto;overflow-y:visible;-webkit-overflow-scrolling:touch;scroll-snap-type:x mandatory;flex:1;min-height:0;width:100%;padding-left:16px;padding-right:16px;box-sizing:border-box}
@media (min-width:640px){.nec-scroller-mobile{padding-left:20px;padding-right:20px}}
@media (min-width:768px){.nec-scroller-mobile{overflow:visible;scroll-snap-type:none;padding-left:0;padding-right:0}}
.nec-scroller-inner{display:flex;flex-direction:column;width:max-content;min-width:100%;box-sizing:border-box}
@media (min-width:768px){.nec-scroller-inner{width:100%;min-width:0}}
.nec-week-row--7{display:flex;flex-direction:row;width:calc(7 * ((100vw - 2.5rem) / 3));min-width:calc(7 * ((100vw - 2.5rem) / 3));box-sizing:border-box}
.nec-week-row--5{display:flex;flex-direction:row;width:calc(5 * ((100vw - 2.5rem) / 3));min-width:calc(5 * ((100vw - 2.5rem) / 3));box-sizing:border-box}
...
.nec-week-row--5 > .nec-day-cell{
  flex:0 0 calc((100vw - 2.5rem) / 3);
  min-width:calc((100vw - 2.5rem) / 3);
  max-width:calc((100vw - 2.5rem) / 3);
...
@media (min-width:768px){
  ...
  .nec-week-row--5 > .nec-grid-head-cell,
  .nec-week-row--5 > .nec-day-cell{
    flex:initial;min-width:0;max-width:none;scroll-snap-align:none
  }
}
...
.nec-day-view{display:flex;flex-direction:column;flex:1;min-height:0;overflow:auto;background:var(--nec-surface)}
.nec-allday{padding:8px 12px;border-bottom:1px solid var(--nec-border);background:var(--nec-week-header-bg);min-height:40px;display:flex;flex-wrap:wrap;gap:6px;align-items:center}
.nec-timeline-wrap{display:flex;flex:1;min-height:var(--nec-day-timeline-height,1248px)}
...
.nec-timeline-canvas{flex:1;position:relative;min-height:var(--nec-day-timeline-height,1248px);background:var(--nec-surface)}
...
.nec-grid-scroll{flex:1;overflow:auto;display:flex;flex-direction:column;min-height:0}
...
export function renderNativeCalendar(mount, options) {
    ...
    let viewMode = isMobile ? 'day' : 'month';
    const chipUnmounts = [];
    ...
    function fillDayCell(cell, cellDate, map, todayKey, maxShow, monthBound, minHeight) {
        const inMonth = monthBound === null ||
    ...
    function renderDayTimeline() {
        ...
        for (const ev of timed) {
            ...
            chipUnmounts.push(mountCalendarChipPeek(inner, ev, {
    ...
    function renderMonthLikeGrid(wrapScroller, rowClass, buildHeader, buildBody, bodyRows) {
        const outer = document.createElement('div');
        if (wrapScroller) {
            outer.className = 'nec-scroller-mobile';
            const inner = document.createElement('div');
            inner.className = 'nec-scroller-inner';
            ...
            headRow.className = rowClass;
            buildHeader(headRow);
            gridScroll.appendChild(headRow);
            for (let r = 0; r < bodyRows; r++) {
                const wr = document.createElement('div');
                wr.className = rowClass;
                buildBody(wr, r);
                gridScroll.appendChild(wr);
            }
        }
    }
    function render() {
        ...
        const useMobileScroller = isMobile && viewMode !== 'day';
        if (viewMode === 'day') {
            renderDayTimeline();
            return;
        }
        if (viewMode === 'month') {
            ...
            renderMonthLikeGrid(useMobileScroller, 'nec-week-row--7', (headRow) => {
        ...
        renderMonthLikeGrid(useMobileScroller, 'nec-week-row--5', (headRow) => {
            for (let i = 0; i < 5; i++) {
