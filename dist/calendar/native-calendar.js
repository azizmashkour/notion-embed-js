...
export function renderNativeCalendar(mount, options) {
    ...
    let viewDropdownCleanup = null;
    const byDay = () => buildEventsByDay(options.events);
    function clearViewDropdownListeners() {
        viewDropdownCleanup?.();
    ...
    function createMobileViewDropdown(viewChoices, currentMode, onSelect) {
        ...
        chevronPath.setAttribute('d', 'M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.94a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z');
    ...
    function renderMonthLikeGrid(rowClass, buildHeader, buildBody, bodyRows) {
    ...
    function render() {
        ...
        if (viewMode === 'month') {
            ...
            renderMonthLikeGrid('nec-week-row--7', (headRow) => {
        ...
        renderMonthLikeGrid('nec-week-row--5', (headRow) => {
            for (let i = 0; i < 5; i++) {
