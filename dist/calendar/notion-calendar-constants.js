...
export const WEEKDAYS_SUN = ['Su', 'Mo', 'Tu', 'We', 'Th', 'Fr', 'Sa'];
export const WORK_HEADERS = ['Mo', 'Tu', 'We', 'Th', 'Fr'];
...
export function calendarViewOptionsForMobile(isMobile) {
    if (isMobile)
        return VIEW_OPTIONS.filter((o) => o.mode !== 'month');
    return [...VIEW_OPTIONS];
}
...
export function rangeStyleForEventId(id) {
    let h = 0;
    for (let i = 0; i < id.length; i++)
        h = (h * 31 + id.charCodeAt(i)) >>> 0;
    return EVENT_RANGE_STYLES[h % EVENT_RANGE_STYLES.length];
}
//# sourceMappingURL=notion-calendar-constants.js.map