...
function positionStyles(cfg: ResolvedLauncherSpec): Record<string, string> {
...
export function mountEmbedLauncher(
  ...
  let inner: RenderedEmbed | null = null;
  ...
  function ensureInner(): RenderedEmbed {
  ...
  const isOpen = (): boolean => openState;
  ...
  const getActiveSegment =
    context.segmentSupport
      ? (): string | null => {
          if (inner?.getActiveSegment) return inner.getActiveSegment();
